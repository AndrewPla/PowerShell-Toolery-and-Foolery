function Install-MSIProgram {
    <#
    .DESCRIPTION
        This installs an msi silently. It will create a logfile in the same directory of the msi.
    #>
    [CmdletBinding()]
    Param (
        $File
    ...
    $logFile = '{0}-{1}.log' -f $(Get-ChildItem $File).fullname, $(get-date -Format yyyyMMddTHHmmss)
    $MSIArguments = @(
        "/i"
        ('"{0}"' -f $file.fullname)
        "/qn"
